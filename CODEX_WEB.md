...
思考、工具调用和正文分别对应 DSH 步骤，后续消息不会覆盖已经完成的思考。投影版本 9 会从 Codex 数据重建旧投影，补齐历史显示。网页发起的轮次按 Codex 条目 ID 转发流式片段，最多合并 80 毫秒；文件变化触发网页推送，合并窗口 25 毫秒，并保留一秒兜底扫描。Desktop 尚未写入文件的 token 无法由文件投影提前读取；若 Codex 连接集中交付整段输出，网页也按实际到达的数据刷新。

同步每 700 毫秒检查变化，历史数据库及 WAL 未变化时复用索引摘要，只读取新增行和新增日志字节。工作区与会话目录改为单次索引，打开网页只读取各日志尾部最多 64 KiB。2026 年 9 月 24 日同一数据集、同一性能分析器下，三次热同步每轮 CPU 耗时由 100–101 毫秒降至 20–21 毫秒；首次重建仍需读取历史数据。
